...
namespace pr9
{
    ...
    {
        // Объявление глобальных параметров CspParmeters и RsaCryptoServiceProvider
        readonly CspParameters _cspp = new CspParameters();
        RSACryptoServiceProvider _rsa;

        // Переменные пути для папок источника, шифрования и дешифрования. 
        // Должно заканчиваться обратной косой чертой.
        const string EncrFolder = @"c:\Encrypt\";
        const string DecrFolder = @"c:\Decrypt\";
        const string SrcFolder = @"c:\docs\";
        // Файл Путь к файлу для открытого ключа
        const string PubKeyFile = @"c:\encrypt\rsaPublicKey.txt";
        // Переменная для пары значений закрытого/открытого ключа
        ...
        private void DecryptFilee(FileInfo file)
        {
        ...
        private void EncryptFilee(FileInfo file)
        {
            // Create instance of Aes for
        ...
        private void EncryptFile_Click(object sender, EventArgs e)
        {
            ...
            {
                // Отобразится диалоговое окно для выбора файла для шифрования.
                ...
                if (_encryptOpenFileDialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    if (fName != null)
                    {
                        ...
                        EncryptFilee(new FileInfo(fName));
                    }
                }
            }
        ...
        private void DecryptFile_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (_decryptOpenFileDialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    if (fName != null)
                    {
                        DecryptFilee(new FileInfo(fName));
                    }
                }
            }
